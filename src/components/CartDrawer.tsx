        ...
        <>
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-border">
              <h2 className="font-display italic text-lg">Your Bag ({items.reduce((s, i) => s + i.quantity, 0)})</h2>
            ...
            {items.length > 0 && (
              <div className="px-6 py-3 bg-aevi-peach">
                {shippingRemaining > 0 ? (
                  <p className="font-body text-xs text-aevi-orange-deep mb-2">
                    You're <span className="font-medium">${shippingRemaining.toFixed(2)}</span> away from free shipping
                  </p>
                ) : (
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4">
              {items.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center">
                  ...
                  <p className="font-display italic text-lg text-foreground mb-1">Your bag is empty</p>
                ...
                <div className="space-y-5">
                  {items.map((item) => (
                    <div key={item.variantId} className="flex gap-4">
                      <div className="w-20 h-20 bg-aevi-peach overflow-hidden flex-shrink-0 rounded-lg">
                        {item.product.node.images?.edges?.[0]?.node && (
                          <img src={item.product.node.images.edges[0].node.url} alt={item.product.node.title} className="w-full h-full object-cover" />
                        )}
                      </div>
                      <div className="flex-1 min-w-0">
                        <h4 className="font-display italic text-sm text-foreground truncate">{item.product.node.title}</h4>
            ...
            {items.length > 0 && (
              <div className="px-6 py-5 border-t border-border">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <span className="font-display italic text-lg text-foreground">{currency} {totalPrice.toFixed(2)}</span>
                </div>
                <button
                  onClick={handleCheckout}
                  disabled={isLoading || isSyncing}
                  className="w-full bg-primary text-primary-foreground font-body text-xs tracking-[0.15em] uppercase py-3.5 hover:bg-aevi-orange-deep transition-colors rounded-lg disabled:opacity-50"
                >
                  {isLoading || isSyncing ? (
